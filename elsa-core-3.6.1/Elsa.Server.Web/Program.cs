...
services
    .AddElsa(elsa =>
    {
        elsa
            ...
            {
                management.UseEntityFrameworkCore(ef => ef.UseSqlServer(builder.Configuration.GetConnectionString("DefaultConnection")));
            ...
            {
                runtime.UseEntityFrameworkCore(ef => ef.UseSqlServer(builder.Configuration.GetConnectionString("DefaultConnection")));
                runtime.UseCache();
        ...
        // HABILITAR AGENTS (dentro do AddElsa):
        elsa.UseAgentPersistence(e => e.UseEntityFrameworkCore(ef => ef.UseSqlServer(builder.Configuration.GetConnectionString("DefaultConnection"))));
        elsa.UseAgentsCore();
        elsa.UseAgentActivities();
        elsa.UseAgentsApi();
        elsa.UseAgents(agents => agents.AddLlmProviders(builder.Configuration));

        // HABILITAR SECRETS (dentro do AddElsa):
        elsa.UseSecretsManagement();
        elsa.UseSecretsScripting();
        elsa.UseSecretsApi();
        elsa.UseSecrets();

        // HABILITAR SUPORTE A MAILKIT E MIMEKIT NAS EXPRESS�ES C# (dentro do AddElsa):
        elsa.UseCSharp(options =>
        {
            options.Assemblies.Add(
                typeof(MimeKit.MimeMessage).Assembly
            );

            options.Assemblies.Add(
                typeof(MailKit.Net.Smtp.SmtpClient).Assembly
            );

            options.Namespaces.Add("MimeKit");
            options.Namespaces.Add("MailKit.Net.Smtp");
        });
        ConfigureForTest?.Invoke(elsa);
    });

// Registrar skills ERP para os agentes
services.AddSingleton<ISkillsProvider, ErpSkillsProvider>();
services.AddTransient<EstoqueSkill>();
services.AddTransient<PedidosSkill>();
services.AddTransient<FornecedoresSkill>();

// Registrar servicos de chat multi-provider
services.AddSingleton<ChatAuditService>();
services.AddSingleton<ChatOrchestrator>();

// add visualizao das tabelas no banco ERP que esto no mesmo banco do elsa
